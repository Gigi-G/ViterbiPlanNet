...
def main():
    ...
    train_loader = DataLoader(
        ...
        num_workers=1, pin_memory=args.pin_memory, drop_last=True
    )
    ...
    main_worker(args.gpu, args, train_loader, valid_loader, diffusion_root)


def main_worker(gpu, args, train_loader, test_loader, diffusion_root):
    args.gpu = gpu
    torch.cuda.set_device(args.gpu)

    # 1. Initialize Models
    temporal_model = temporal.TemporalUnet(
        args.class_dim + args.action_dim + args.observation_dim, dim=256, dim_mults=(1, 2, 4)
    )
    ...
    checkpoint_dir = os.path.join(diffusion_root, f'{args.dataset}_{args.split}')
    ...
    max_eva = 0
    old_max_epoch = 0
    save_max = os.path.join(diffusion_root, f'{args.dataset}_{args.split}_max')
