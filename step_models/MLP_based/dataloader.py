...
class PlanningDataset(Dataset):
    ...
    def load_data(self):
        ...
        for video_info in video_info_dict:
            ...
            try:
                ...
                saved_features = np.load(video_info["id"]["feature"], allow_pickle=True)
            if self.PKG_labels:
    ...
    def __getitem__(self, idx, return_video_id = False):
        ...
        if return_video_id:
            ...
            return torch.as_tensor(states, dtype=torch.float32), torch.as_tensor(actions, dtype=torch.long), torch.as_tensor(tasks, dtype=torch.long)#, torch.as_tensor(video_len, dtype=torch.long)
