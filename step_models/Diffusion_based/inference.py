...
def test(val_loader, model, args, mode):
    ...
    for batch_states, y, _ in val_loader:
        ...
        with torch.no_grad():
            ...
            actions_pred = output[:, :, :args.action_dim].contiguous()
...
def main_worker(gpu, args, test_loader, mode):
    ...
    if args.resume:
        checkpoint_dir = os.path.join(args.checkpoint_root, f'{args.dataset}_{args.split}_max')
