...
class PlanningDataset(Dataset):
    ...
    def load_data(self):
        ...
        for video_info in video_info_dict:
            ...
            feature_path = video_info["id"]["feature"]
            if not os.path.exists(feature_path):
                feature_path = feature_path.replace("./dataset", "../../dataset", 1)
            try:
                saved_features = np.load(feature_path, allow_pickle=True)["frames_features"]
            except:
                saved_features = np.load(feature_path, allow_pickle=True)
            ...
            if all_features == []:
                ...
                self.no_valid_videos.append(video_id)
                continue
    ...
    def __getitem__(self, idx, return_video_id = False):
        ...
        if return_video_id:
            ...
            # Return the video_id as None or some default value
