    ...
    def train(self, n_train_steps, if_calculate_acc, args, scheduler):
        ...
        for step in range(n_train_steps):
            for i in range(self.gradient_accumulate_every):
                batch_states, y, _ = next(self.dataloader)
                ...
                action_label_onehot = torch.zeros((video_label.size(0), self.model.action_dim))
                # [bs*T, ac_dim]
        ...
        if if_calculate_acc:
            with torch.no_grad():
                ...
                actions_pred = output[:, :, :self.model.action_dim]\
