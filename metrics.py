...
def miou(pred, target):
    ...
    for i in range(len(pred)):
        pred_inst_set.update(pred[i])
        target_inst_set.update(target[i])
        inter = pred_inst_set.intersection(target_inst_set)
        union = pred_inst_set.union(target_inst_set)
        MIoU = 100.0 * len(inter) / len(union)
        MIoU_list.append(MIoU)
...
def first_action_accuracy(pred_paths, gt_paths):
    """
    Computes the accuracy of the first action.

    Args:
        pred_paths (list of lists): Predicted action sequences.
        gt_paths (list of lists): Ground truth action sequences.

    Returns:
        float: Accuracy of the first action.
    """
    total = len(gt_paths)
    correct = sum(1 for pred, gt in zip(pred_paths, gt_paths) if pred[0] == gt[0])
    return (correct / total) * 100


def last_action_accuracy(pred_paths, gt_paths):
    """
    Computes the accuracy of the last action.
    ...
    Returns:
        float: Accuracy of the last action.
    """
    total = len(gt_paths)
    correct = sum(1 for pred, gt in zip(pred_paths, gt_paths) if pred[-1] == gt[-1])
    return (correct / total) * 100